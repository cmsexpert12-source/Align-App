...
async function walk(label, fn, models, key, system, prompt) {
  if (!key) {
...
export default async function handler(req, res) {
  ...
  for (const p of prefer) {
    try {
      const out = p === "gemini"
        ? await walk("Gemini", callGemini, GEMINI_MODELS, geminiKey, system, prompt)
        : await walk("Groq", callGroq, GROQ_MODELS, groqKey, system, prompt);
